...
class JokeModule implements AbstractModule {
  static void configure(Injector injector) {
    ...
    injector.map<LoadRandomJokeCubit>(
      (i) => LoadRandomJokeCubit(
        i.get<GetRandomJokeUseCase>(),
        i.get<GetRandomJokePerCategoryUseCase>()
      ),
      isSingleton: true
    );
    ...
    injector.map<GetRandomJokePerCategoryUseCase>(
      (i) => GetRandomJokePerCategoryUseCase(i.get<JokeRepository>()),
      isSingleton: true
    );
  }
}